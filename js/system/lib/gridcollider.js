	...
	function(Vector2) {
		...
		GridCollider.prototype.checkHorizontalCollision = function(move, pos, collision) {
			...
			for (var tileY = firstTileY; tileY < lastTileY; tileY++) {
				if (this.map.blocked(new Vector2(tileX, tileY))) {
					collision.x = true;
			...
			var a = this.subject.getArea().moved(new Vector2(p.x, pos.y));
		...
		GridCollider.prototype.checkVerticalCollision = function(move, pos, collision) {
			...
			var a = this.subject.getArea().moved(new Vector2(pos.x, p.y));
			for (var i in this.obstacles) {
				var o = this.obstacles[i];
